...
namespace SceneGraph {
	...
	void StaticGeometry::Save(NodeDatabase &db)
	{
		...
		for (auto mesh : m_meshes) {
			...
			uint32_t attribCombo = 0;
			for (uint32_t i = 0; i < Graphics::MAX_ATTRIBS; i++)
			...
			const uint32_t posOffset = vbDesc.GetOffset(Graphics::ATTRIB_POSITION);
			const uint32_t nrmOffset = vbDesc.GetOffset(Graphics::ATTRIB_NORMAL);
			const uint32_t uv0Offset = vbDesc.GetOffset(Graphics::ATTRIB_UV0);
			const uint32_t tanOffset = hasTangents ? vbDesc.GetOffset(Graphics::ATTRIB_TANGENT) : 0;
			const uint32_t stride = vbDesc.stride;
			db.wr->Int32(vbDesc.numVertices);
			uint8_t *vtxPtr = mesh.vertexBuffer->Map<uint8_t>(Graphics::BUFFER_MAP_READ);
			if (hasTangents) {
				for (uint32_t i = 0; i < vbDesc.numVertices; i++) {
			...
			//indices
			const uint32_t *indexPtr = mesh.indexBuffer->Map(Graphics::BUFFER_MAP_READ);
			const uint32_t numIndices = mesh.indexBuffer->GetSize();
	...
	StaticGeometry *StaticGeometry::Load(NodeDatabase &db)
	{
		...
		const uint32_t numMeshes = rd.Int32();
		for (uint32_t mesh = 0; mesh < numMeshes; mesh++) {
			//material
			...
			//vertex format check
			const uint32_t vtxFormat = db.rd->Int32();
			...
			const uint32_t posOffset = vtxBuffer->GetDesc().GetOffset(Graphics::ATTRIB_POSITION);
			const uint32_t nrmOffset = vtxBuffer->GetDesc().GetOffset(Graphics::ATTRIB_NORMAL);
			const uint32_t uv0Offset = vtxBuffer->GetDesc().GetOffset(Graphics::ATTRIB_UV0);
			const uint32_t tanOffset = hasTangents ? vtxBuffer->GetDesc().GetOffset(Graphics::ATTRIB_TANGENT) : 0;
			const uint32_t stride = vtxBuffer->GetDesc().stride;
			...
			//index buffer
			const uint32_t numIndices = db.rd->Int32();
			...
			uint32_t *idxPtr = idxBuffer->Map(BUFFER_MAP_WRITE);
			for (uint32_t i = 0; i < numIndices; i++)
