...
public:
		...
		SBaseRequest(v0_, v1_, v2_, v3_, cn, depth_, sysPath_, patchID_, edgeLen_, fracStep_, pTerrain_)
	{
		...
		m_heights = std::vector<double>(numVerts);
		m_normals = std::vector<vector3f>(numVerts);
		m_colors = std::vector<Color3ub>(numVerts);

		const int numBorderedVerts = NUMVERTICES(edgeLen_ + (BORDER_SIZE * 2));
		m_tmpBorderHeights = std::vector<double>(numBorderedVerts);
		m_tmpBorderVertexs = std::vector<vector3d>(numBorderedVerts);
	}
...
private:
	...
	std::vector<double> m_tmpBorderHeights;
	std::vector<vector3d> m_tmpBorderVertexs;
};
...
public:
		...
		SBaseRequest(v0_, v1_, v2_, v3_, cn, depth_, sysPath_, patchID_, edgeLen_, fracStep_, pTerrain_)
	{
		const int numVerts = NUMVERTICES(edgeLen_);
		for (int i = 0; i < 4; ++i) {
			m_heights[i] = std::vector<double>(numVerts);
			m_normals[i] = std::vector<vector3f>(numVerts);
			m_colors[i] = std::vector<Color3ub>(numVerts);
		}
	...
	// Generates full-detail vertices, and also non-edge normals and colors
	void GenerateBorderedData();

	void GenerateSubPatchData(const int quadrantIndex,
		const vector3d &v0, const vector3d &v1, const vector3d &v2, const vector3d &v3,
		const int edgeLen, const int xoff, const int yoff, const int borderedEdgeLen);

	// these are created with the request and are given to the resulting patches
	std::vector<vector3f> m_normals[4];
	std::vector<Color3ub> m_colors[4];
	std::vector<double> m_heights[4];
...
public:
	struct SSplitResultData {
		SSplitResultData() :
			patchID(0) {}
		SSplitResultData(std::vector<double> &heights_, std::vector<vector3f> &n_, std::vector<Color3ub> &c_, const vector3d &v0_, const vector3d &v1_, const vector3d &v2_, const vector3d &v3_, const GeoPatchID &patchID_) :
			m_heights(std::move(heights_)),
			m_normals(std::move(n_)),
			m_colors(std::move(c_)),
			v0(v0_),
			v1(v1_),
			v2(v2_),
			v3(v3_),
			patchID(patchID_)
		{}
		SSplitResultData(const SSplitResultData &r) = delete;
...
public:
	SSingleSplitResult(const SSingleSplitResult &r) = delete;
	...
	void addResult(std::vector<double> &h_, std::vector<vector3f> &n_, std::vector<Color3ub> &c_, const vector3d &v0_, const vector3d &v1_, const vector3d &v2_, const vector3d &v3_, const GeoPatchID &patchID_)
	{
...
public:
	SQuadSplitResult(const SQuadSplitResult &r) = delete;
	SQuadSplitResult(const int32_t face_, const int32_t depth_) :
		SBaseSplitResult(face_, depth_)
	{}

	void addResult(const int kidIdx, std::vector<double> &h_, std::vector<vector3f> &n_, std::vector<Color3ub> &c_, const vector3d &v0_, const vector3d &v1_, const vector3d &v2_, const vector3d &v3_, const GeoPatchID &patchID_)
	{
